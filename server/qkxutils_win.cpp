...
bool QKxUtils::createWinProcess(const QString &path, const QStringList &env, void *process)
{
    PROCESS_INFORMATION *pInfo = (PPROCESS_INFORMATION)process;
    HANDLE hToken;
    ...
    DWORD dwCreationFlags = NORMAL_PRIORITY_CLASS | CREATE_NEW_CONSOLE | CREATE_UNICODE_ENVIRONMENT;
    ok = CreateProcessAsUserW(hToken, (LPWSTR)path.constData()/*"c:\\windows\\system32\\notepad.exe"*/, NULL, NULL, NULL, FALSE,
                             dwCreationFlags, environment, NULL, &si, pInfo);
...
bool QKxUtils::startDetachProcess(const QString &path, const QMap<QString, QString> &env)
{
...
void QKxUtils::resetThreadDesktop()
{
    HDESK hDeskInput = OpenInputDesktop(0, FALSE, GENERIC_ALL);
    if (hDeskInput == NULL) {
        return;
...
void QKxUtils::checkFirewall()
{
    // must be run on common user mode, or would not popup firewall request.
    Q_ASSERT(!QKxUtils::isRunAsAdmin());
    QTcpServer tmp;
    tmp.setProxy(QNetworkProxy::NoProxy);
    tmp.listen();
}
