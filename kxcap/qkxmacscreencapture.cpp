...
QKxMacScreenCapture::QKxMacScreenCapture(int screenIndex, const QRect &rt, const QRect &imgRt, QObject *parent)
    : QKxScreenCapture(screenIndex, rt, imgRt, parent)
{

}

int QKxMacScreenCapture::running()
{
    QKxMacCapture cap(m_screenIndex, m_imageRect, m_screenRect);
    if(!cap.reset()) {
        return -999;
    }
    qDebug() << "QKxMacScreenCapture::running";
    m_cap = &cap;
    int lastErr = 0;
    qint8 lastBitUsed = 0;
    while(true) {
        ...
        if(n == 0) {
            qDebug() << "QKxMacScreenCapture::running-timeout:" << QDateTime::currentMSecsSinceEpoch();
            continue;
        }
